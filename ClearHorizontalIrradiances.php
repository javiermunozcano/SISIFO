...
function ClearHorizontalIrradiances($Eo, $ws, $w, $SUNPOS, $AMI, $METEO, $TIME) {
	...
	//foreach ($TIME['SimulationDays'] as $dm) 
		{
	//	$d = $dm-1;  //AMP: En Matlab estaba puesto así. Revisaremos si está mal.
		...
		for ($h = 0; $h < $TIME['Nsteps']; $h++) 
			...
			if ( abs($w[$d][$h]) >= abs($ws[$d]) )
				...
				$D0[$d][$h]=0;
		            ...
		            $B0[$d][$h]=1367 *$Eo[$d] *$costetazs[$d][$h] *exp(-0.8662 *$Tlkd[$d] *$AMI[$d][$h] *$deltaR[$d][$h]);
		            ...
		            //Global hrizontal clear irradiance
