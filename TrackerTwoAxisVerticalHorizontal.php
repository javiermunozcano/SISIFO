...
function TrackerTwoAxisVerticalHorizontal($SUNPOS, $w, $ws, $HI, $ANISO, $SITE, $PVGEN, $OPTIONS, $TIME) {
...
//PVGEN
	...
	$NBT=$PVGEN['NBT'];
	//This tracker
	$Azimut_MAX=$PVGEN['Track2VH']['Azimut_MAX'];
	$Inclination_MAX=$PVGEN['Track2VH']['Inclination_MAX'];
	$LEO=$PVGEN['Track2VH']['LEO'];
	$LNS=$PVGEN['Track2VH']['LNS'];
	$ALARG=$PVGEN['Track2VH']['ALARG'];
	$RSEV=$PVGEN['Track2VH']['RSEV'];
	$RSEH=$PVGEN['Track2VH']['RSEH'];
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	for ($d = 0; $d < $TIME['Ndays']; $d++) 
		...
		for ($h = 0; $h < $TIME['Nsteps']; $h++) 
			...
			// The analysis process is: ideal value, design limitations, analysis of shadows, back-tracking corrections
				
			//Ideal values
			$beta[$d][$h] = $tetazs[$d][$h];
			$alfa[$d][$h] = $fis[$d][$h];
			
			//Limits the inclination angle to constructive maximum value
			...
			if ( $FSGTSE[$d][$h] > 0 )
				...
				// Consideration of the shadows projected by a tracker located at the SW.
				// geometric:
				// Azimuth at the beginning of the shadow
				...
				// Consideration of the shadows projected by a tracker located at the S.
				// geometric:
				// Azimuth at the beginning of the shadow
				$tanfiscss= -1/$LNS;
				//Azimuth at the end of the shadow
				...
				// Correction backtracking azimuth angle:
				// First, this version is based on the conjecture (probably true for NBGV = NBGH) that the best option
				// to back-tracking corresponds precisely to the lower angle correction.
				// On the other hand, note the choice: when is for beta, eleccionbeta = 1; when is for alpha, eleccionbeta = -1. 
				// When it refers to the shadow tracker located southeast and southwest eleccionbetaseo use = 1 and eleccionbetaseo= -1 respectively 
				...
				//To avoid the shadows projection by a tracker located at E
				if ($w[$d][$h] <= 0 )
					{
				...
				//To avoid the shadows projection by a tracker located at W
				if ( $w[$d][$h] >= 0 )
					{
				...
				//To avoid the shadows projection by a tracker located at SE
				// Azimuth canceled because of implant there is a rotation of 180-degree , but keep in inclination
				...
				//To avoid the shadows projection by a tracker located at SW
				if ( ($RSEH == 1) && ($gammas[$d][$h] > 0) )
					{
					if ( tan($fis[$d][$h]) >= $tanfiscsso )
						{
				...
				//Limits, again, the inclination angle to the maximum value constructive
				if ( $beta[$d][$h] > $Inclination_MAX*pi()/180 )
					{
					$beta[$d][$h]=$Inclination_MAX*pi()/180;
					}
				...
				//Isotropic, cicumsolar and horizon components of the diffuse irradiation
				if ( abs($w[$d][$h]) >= abs($ws[$d]) )
					{
					$Diso[$d][$h]=0;
					$Dcir[$d][$h]=0;
				...
				// Correction direct irradiation factor 
				$FCB[$d][$h]=(1-exp(-$costetas[$d][$h]/$ar))/(1-exp(-1/$ar));
				
				// Correction diffuse irradiation factor 
				...
				// Correction albedo irradiation factor 
				if ( $beta[$d][$h] == 0 )
					{
					$FCR[$d][$h]=0;
					}else
				...
				// Maximum limitation shadow factor to unit
